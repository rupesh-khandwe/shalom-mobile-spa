...
const VerseCount = ({route, navigation}) => {
  ...
  useEffect(() => {
    console.log(route.params.chapterId+"=="+route.params.bibleId);
    axios
    .get(FETCH_ALL_BOOK_URL+"/"+route.params.bibleId+"/chapters/"+route.params.chapterId, { 
      headers: { 'api-key': API_KEY, 'content-type': 'application/json'}}) 
    .then((res) => {
        console.log(
          res.data.data.verseCount);
          let loopData = [];
          var counter = 1;
          var loopCycle= (res.data.data.verseCount) /6;
          let dataTableArray = [];
          let test = [];

          const elementButton = (value) => ( 
            <TouchableOpacity onPress={() => navigation.push('Verse', {chapterId: route.params.chapterId +"."+value, bibleId: route.params.bibleId})} >
              <View >
